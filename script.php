...
class pkg_vestnikInstallerScript
{
	/**
	...
	protected function checkCompatible()
	{
		...
		{
			JFactory::getApplication()->enqueueMessage(JText::sprintf('PKG_VESTNIK_ERROR_COMPATIBLE_JOOMLA',
		...
		{
			$app->enqueueMessage(Text::sprintf('PKG_VESTNIK_ERROR_COMPATIBLE_PHP', $this->minimumPhp),
				'error');
		...
		{
			$app->enqueueMessage(Text::sprintf('PKG_VESTNIK_ERROR_COMPATIBLE_JOOMLA', $this->minimumJoomla),
				'error');
		...
		if ($serverType == 'mysql' && stripos($serverVersion, 'mariadb') !== false)
		{
			...
			{
				$app->enqueueMessage(Text::sprintf('PKG_VESTNIK_ERROR_COMPATIBLE_DATABASE',
